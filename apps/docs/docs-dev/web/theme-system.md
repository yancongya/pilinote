...
设置页在主题系统之上再抽了一层页面级设计系统，用来统一多 tab 设置页的结构、节奏和按钮语义。当前这层系统主要用于 `SettingsPage` 及其各个设置 tab。
...
**设计原则**：
- 表单项默认采用纵向排布，必要时再切换为横向 row
- 主按钮强调动作，次要按钮采用 outlined / tonal 风格
- 图标按钮控制在最小可点击范围，避免明显立体感

### 4. 设计语言

这套设置页设计语言偏向现代移动端设置页和轻量 Material：

- 减少纯色填充块
- 减少嵌套容器和深色背景层
- section、列表、弹窗保持统一圆角和边框节奏
- tab 使用下划线或轻高亮，而不是厚重浮层
- 关键操作保留层级感，但不做强立体阴影
...
如果后续新增设置页，默认先复用这套页面级组件，再补单页局部样式。
