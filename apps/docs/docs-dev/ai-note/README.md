...
其中媒体库侧的 `AiNoteModal` 负责发起分析和展示运行状态，系列视频会在弹窗内按单集队列逐个复用现有单集流水线。`AiNoteModal` 和详情页的 `AiNotePanel` 会共用同一套本地快照缓存，关闭或刷新后都可以先回读上次的状态与日志，再向服务端回填最新结果。
...
- **媒体库笔记类型适配**：图文/笔记入口不再沿用旧的 `video` 流水线，前端会强制传入 `image_text` 模式。
- **历史记录回填**：后端启动时会回填旧 `ai_notes.pipeline_mode`，将符合图文/笔记特征的记录迁移为 `image_text`。
- **系列视频队列**：媒体库里的系列视频不再走独立聚合流水线，而是在 `AiNoteModal` 中按 episode 队列逐集复用单集视频分析流程。
- **图文结果回填**：`GET /api/note/by-video` 现在按 `updated_at` / `created_at` 取最新记录，避免旧失败笔记把页面和重试入口拖回过期状态。
- **图文稳定性修复**：图文分析路径中的 `pipeline_mode` 已完整透传到后端分析入口，重新生成和首次分析都会稳定走 `image_text` 分支。
- **详情页展示顺序**：视频详情页中，作者面板位于封面下方，视频简介位于作者面板下面，下载状态组件位于评论区之后。
- **加载与错误态**：详情页刷新时优先展示骨架屏，并使用本地缓存兜底，减少“加载中...”和短暂错误页闪现。
- **评论区样式**：评论区标题去掉数量显示，并收敛为更统一的面板样式。
...
- 视频信息在上方
- AI 笔记结果在下方（无分析则不显示）
- 刷新时先显示骨架屏，不显示纯文本“加载中...”
- 若接口短暂失败但存在缓存，会继续显示最近一次成功的详情内容
...
| AiNoteModal | `src/components/ai/AiNoteModal.tsx` | 媒体库分析弹窗（单集 / 系列队列，读取 prompt 卡片与 runtime state） |
...
- **媒体库卡片**：点击右下角 AI 按钮 → 打开 `AiNoteModal` → 单集直接分析 / 系列按 episode 队列执行
