...
`MainLayout` 现在承载两类重要页面入口：

- `SettingsPage`：统一 settings 设计系统
- 收藏页 / 稍后再看页 / 历史记录页：统一 `MediaListShell` 壳层

这意味着布局层不再关心每个页面内部怎么画卡片，而只负责把页面交给对应的统一壳层组件。
...
`SettingsPage` 已经从简单的页面组件升级为统一的设置页设计系统入口，包含页面壳、section、字段块、状态组件和 tab 切换过渡。

### 组件职责

- `SettingsPageShell`：统一页面背景、内容宽度和滚动区域
- `SettingsSection`：统一面板组标题、动作区和内容区
- `SettingsField`：统一输入项和说明文本
- `SettingsToggleRow`：统一开关行排版
- `SettingsTabPanel`：统一 tab 切换时的淡入与轻微缩放

### 视觉原则

- 避免在设置页内再嵌套一层重背景卡片
- 使用分隔线表达层级，而不是依赖大面积纯色块
- 保持按钮、标签和状态徽章的轻量 Material 风格
- 优先让移动端阅读密度更高、层级更清楚
