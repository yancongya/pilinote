...
class DownloadService:
    ...
    def create_download_task(
        ...
        with SessionLocal() as db:
            download = Download(
                ...
                sessdata=sessdata,
                enable_subtitle=int(enable_subtitle)
    ...
    def get_downloads_by_bvid(
        self, bvid: str, status: Optional[str] = None
    ) -> list[Download]:
        """根据bvid获取下载任务"""
        with SessionLocal() as db:
        ...
        try:
            ...
            try:
                ...
                uploader_info = await bilibili_service.get_uploader_info(
                    download.uploader_mid, download.sessdata or ""
                )
                ...
                if uploader_info.get("success"):
                    ...
                    logger.warning(
                        f"Failed to get uploader info: {uploader_info.get('message')}"
                    )
                    return False
            finally:
    ...
    async def _process_completed_download(
        self, download_id: str, temp_dir: Path, final_dir: Path, storage_settings
    ):
        """
        处理已完成的下载 - 移动文件并清理
        ...
        try:
            ...
            if video_files:
                ...
                with SessionLocal() as db:
                    download = (
                        db.query(Download).filter(Download.id == download_id).first()
                    )
                    if download:
                        ...
                        if download and download.enable_nfo:
                            ...
                            try:
                                video_info = await bilibili_service.get_video_info(
                                    download.bvid, download.sessdata or ""
                                )
                                if video_info.get("success"):
                                    video_data = video_info.get("data", {})
                                    description = video_data.get("desc")
                                    video_stats = {
                                        "play": video_data.get("stat", {}).get(
                                            "view", 0
                                        ),
                                        "like": video_data.get("stat", {}).get(
                                            "like", 0
                                        ),
                                        ...
                                        "share": video_data.get("stat", {}).get(
                                            "share", 0
                                        ),
                                        "danmaku": video_data.get("stat", {}).get(
                                            "danmaku", 0
                                        ),
                                        "reply": video_data.get("stat", {}).get(
                                            "reply", 0
                                        ),
                                    }
                            except Exception as e:
                ...
                try:
                    ...
                    with SessionLocal() as db:
                        ...
                        if download:
                            logger.info(
                                f"enable_cover={download.enable_cover}, thumbnail_url={download.thumbnail_url}"
                            )
                            logger.info(f"video_dir={video_dir}")

                            if download.enable_cover and download.thumbnail_url:
                                logger.info(f"Starting thumbnail download...")
                                success = await self._download_thumbnail(
                                    download, video_dir
                                )
                                logger.info(f"Thumbnail download result: {success}")
                            else:
                                logger.info(
                                    f"Thumbnail download disabled: enable_cover={download.enable_cover}"
                                )
                except Exception as e:
                    logger.error(f"Failed to download thumbnail: {e}")
                    import traceback
                ...
                try:
                    ...
                    with SessionLocal() as db:
                        ...
                        if download:
                            logger.info(
                                f"enable_avatar={download.enable_avatar}, uploader_mid={download.uploader_mid}"
                            )
                            logger.info(f"video_dir={video_dir}")

                            if download.enable_avatar and download.uploader_mid:
                                logger.info(f"Starting avatar download...")
                                success = await self._download_avatar(
                                    download, video_dir
                                )
                                logger.info(f"Avatar download result: {success}")
                            else:
                                logger.info(
                                    f"Avatar download disabled: enable_avatar={download.enable_avatar}"
                                )
                except Exception as e:
                    ...
                    with SessionLocal() as db:
                        ...
                        if download:
                            ...
                            if download.enable_subtitle:
                                ...
                                logger.info(
                                    f"Subtitle download disabled: enable_subtitle={download.enable_subtitle}"
                                )
                except Exception as e:
                ...
                video_dir = final_dir
                logger.warning(
                    f"No video files detected, fallback metadata output dir: {video_dir}"
                )
    ...
    async def _handle_failed_or_cancelled_download(
        self, download_id: str, temp_dir: Path, storage_settings, status: str
    ):
        """
        处理失败或取消的下载
                ...
                with SessionLocal() as db:
                    ...
                    if download:
                        ...
                        video_files = [
                            ...
                            if f.is_file()
                            and f.suffix in [".mp4", ".flv", ".mkv", ".webm"]
                        ]
                        if video_files:
                            # 更新文件路径为保留路径
    ...
    async def _download_video(self, download_id: str):
        ...
        try:
            ...
            series_root_name = _sanitize_folder_name(
                _derive_series_name(raw_title), safe_title
            )
            ...
            # 处理已完成的下载（移动文件到最终目录）
            print(
                f"DEBUG: About to call _process_completed_download with download_id={download_id}"
            )
            print(f"DEBUG: temp_download_dir={temp_download_dir}")
            print(f"DEBUG: final_dir={final_dir}")
            print(
                f"DEBUG: Using download path from settings: {storage_settings.download_path}"
            )
    ...
    async def _get_subtitles(self, download: Download) -> list:
        ...
        try:
            ...
            try:
                logger.info(
                    f"Getting subtitles for aid={download.aid}, cid={download.cid}"
                )
    ...
    def _build_subtitle_filename(
        ...
        language = subtitle.get("language") or self._normalize_subtitle_language(
            subtitle.get("raw", {})
        )
        source = subtitle.get("source") or self._classify_subtitle_source(
            subtitle.get("raw", {})
        )
    ...
    async def _download_subtitle(
        self, download: Download, output_dir: Path, subtitle_lan: str
    ) -> bool:
        """
        下载指定语言的字幕并转换为SRT格式
        ...
        try:
            ...
            subtitle_candidates = self._get_subtitle_candidates(
                subtitles, [subtitle_lan]
            )
            subtitle_info = (
                subtitle_candidates[0]["raw"] if subtitle_candidates else None
            )
    ...
    async def _download_preferred_subtitles(
        self, download: Download, output_dir: Path
    ) -> dict:
        """只下载中英双语字幕，返回成功/失败统计"""
        subtitles = await self._get_subtitles(download)
        if not subtitles:
            logger.info(
                f"No subtitles found for this video: aid={download.aid}, cid={download.cid}"
            )
            return {"downloaded": 0, "attempted": 0, "languages": []}

        preferred_languages = ["zh-CN", "en-US"]
        downloadable_subtitles = self._get_subtitle_candidates(
            subtitles, preferred_languages
        )
        ...
        for index, subtitle in enumerate(downloadable_subtitles):
            ...
            success = await self._download_subtitle_from_info(
                download, output_dir, subtitle, index
            )
            if success:
                downloaded += 1
    ...
    async def _download_subtitle_from_info(
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            ...
            subtitle_path = self._build_subtitle_filename(
                output_dir, subtitle, subtitle_index
            )
            logger.info(f"Saving subtitle to: {subtitle_path}")
            subtitle_path.write_text(srt_content, encoding="utf-8")
            logger.info(f"Successfully downloaded subtitle: {subtitle_path}")
    ...
    async def download_subtitle_with_ytdlp(
        self, video_url: str, output_dir: Path, langs: List[str] = None
    ) -> Optional[Dict]:
        """
        使用 yt-dlp 下载B站字幕

        Args:
            video_url: 视频URL或bvid
            output_dir: 输出目录
            langs: 优先语言列表

        Returns:
            包含字幕内容和语言信息的字典，或None
        """
        if langs is None:
        ...
        ydl_opts = {
            ...
            "skip_download": True,
            "outtmpl": str(output_dir / f"{video_stem}.%(ext)s"),
            "quiet": True,
        }
        ...
        try:
            ...
            try:
                ...
                if cookies:
                    ...
                    for c in cookies:
                        cookie_content += (
                            f".bilibili.com\tTRUE\t/\tFALSE\t0\t{c.name}\t{c.value}\n"
                        )

                    cookie_path = output_dir / "cookies.txt"
                    cookie_path.write_text(cookie_content, encoding="utf-8")
                    ydl_opts["cookiefile"] = str(cookie_path)
                    logger.info(f"使用数据库 cookie 文件下载字幕")
            finally:
                db.close()
        except Exception as e:
            logger.warning(f"获取 cookie 失败，将使用匿名方式: {e}")
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if "data" in sub_info and sub_info["data"]:
                    return {
                        ...
                        "content": sub_info["data"],
                        "format": "srt",
                    }

                subtitle_file = None
                for ext in ["srt", "json3", "vtt"]:
                    candidate = output_dir / f"{video_id}.{detected_lang}.{ext}"
                    if candidate.exists():
                        subtitle_file = str(candidate)
                        break

                if not subtitle_file:
                    logger.warning(f"字幕文件未找到")
                    return None
            ...
            logger.error(f"yt-dlp 字幕下载失败: {e}")
            return None
        finally:
    ...
    async def _download_preferred_subtitles(
        ...
        """只下载中英双语字幕，优先使用 yt-dlp"""
        video_url = f"https://www.bilibili.com/video/{download.bvid}"
        result = await self.download_subtitle_with_ytdlp(
            video_url=video_url,
            output_dir=output_dir,
            langs=["zh-Hans", "zh-CN", "ai-zh", "en"],
        )

        if result:
            video_stem = (
                Path(download.file_path).stem if download.file_path else download.bvid
            )
            srt_path = output_dir / f"{video_stem}.zh-CN.srt"

            if result.get("format") == "json3":
                content = self._convert_json3_to_srt(result["content"])
            else:
            ...
            return {
                "downloaded": 1,
                "attempted": 1,
                "languages": [result.get("language", "zh-CN")],
            }
