...
class DownloadManager:
    ...
    async def start_task(self, download_id: str) -> bool:
        ...
        if download.status == DownloadStatus.PAUSED.value:
            ...
            or download.status == DownloadStatus.FAILED.value
        ):
            # 重置任务状态并重新开始
            ...
            logger.warning(
                f"Cannot start download {download_id} with status {download.status}"
            )
            return False
    ...
    def _update_status(
        self, download_id: str, status: str, error_message: Optional[str] = None
    ):
        """更新下载状态"""
        with SessionLocal() as db:
    ...
    async def _process_queue(self):
        ...
        while self._running:
            try:
                ...
                # 检查并发限制
                active_count = len(
                    [t for t in self.active_downloads.values() if not t.done()]
                )
    ...
    async def _execute_download(self, download_id: str):
        ...
        if not acquired:
            ...
            self._update_status(
                ...
                "Failed to acquire download resource (timeout)",
            )
            return

        try:
            logger.info(
                f"Starting download execution for {download_id}, bvid: {download.bvid}, title: {download.title}"
            )
            self._update_status(download_id, DownloadStatus.DOWNLOADING.value)
            ...
            if not os.path.isabs(base_download_path):
                base_download_path = os.path.abspath(base_download_path)
            ...
            if download.aid:
                ...
                # 单个视频：使用完整标题
                safe_title = re.sub(
                    r'[<>:"/\\|?*\x00-\x1f]', "", download.title
                ).strip()
                if not safe_title:
                    safe_title = "video"
                logger.info(f"Single video, using title: {safe_title}")

            # 构建输出路径：使用配置的下载路径 + 合集名称作为目录名
            output_path = os.path.join(base_download_path, safe_title)
                ...
                for root, dirs, files in os.walk(output_path):
                    for file in files:
                        if file.endswith((".mp4", ".flv", ".mkv", ".webm")):
                            ...
                            logger.info(
                                f"Found video file: {full_path}, size: {file_size} bytes"
                            )
            ...
            if video_files:
                ...
                if download.aid:
                    ...
                    with SessionLocal() as db:
                        series_downloads = (
                            db.query(Download)
                            .filter(Download.aid == download.aid)
                        ...
                        for idx, (file_path, file_size) in enumerate(
                            ...
                            if os.path.exists(file_path) and not os.path.exists(
                                ...
                                logger.info(
                                    f"Renamed: {os.path.basename(file_path)} -> {new_filename}"
                                )
                                # 更新文件路径
                ...
                with SessionLocal() as db:
                    ...
                    if download:
                        ...
                        db.commit()
                        logger.info(
                            f"Updated file path for {download_id}: {main_file}, total size: {total_size}"
                        )
            else:
                logger.warning(
                    f"No video files found for {download_id} in {output_path}"
                )
