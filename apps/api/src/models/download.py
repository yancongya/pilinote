...
class Download(Base):
    ...
    )  # 来源类型：favorite/watchlater/direct
    ...
    quality = Column(
        Integer, default=64
    )  # 视频质量 (16=360P, 32=480P, 64=720P, 80=1080P, 112=1080P+, 116=4K)
    ...
    ai_note_id = Column(String, nullable=True, index=True)  # 关联的 AI 笔记 ID

    ai_summary = Column(Text, nullable=True)  # AI 总结摘要

    ai_markdown = Column(Text, nullable=True)  # 完整 Markdown

    ai_style = Column(String(50), nullable=True)  # 使用的风格
    ...
    ai_error = Column(Text, nullable=True)  # AI 分析错误信息

    transcript = Column(Text, nullable=True)  # 字幕转写文本

    transcript_lang = Column(String(10), nullable=True)  # 转写语言
