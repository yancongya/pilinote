...
`MediaListTopBar` 是收藏页、稍后再看页、历史记录页共用的顶部导航筛选条组件，用来把标题、数量、页面动作和筛选控件组合成一个全宽顶部区域。
...
- 作为顶部内容块，交给 `MediaListShell` 的 header 容器吸顶
- 初始渲染时与 shell 顶部边界对齐，由 shell 决定贴边和吸顶行为
- 让导航和筛选条填满列表内容区的左右边缘
