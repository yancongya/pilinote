...
`MediaListShell` 是收藏页、稍后再看页、历史记录页共用的列表壳层组件，用来统一这些“历史型视频列表”的加载态、空态、错误态、滚动容器和顶部区域。
...
- 提供 `topBar` 插槽，接入 `MediaListTopBar` 后可让导航和筛选条吸顶
...
- `MediaListTopBar` 负责 sticky、全宽和左右对齐
