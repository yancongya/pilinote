...
当前详情页已经统一为 PC / 手机共用的单列流式布局，不再走桌面双栏容器。页面结构按从上到下的阅读顺序排列：
...
顶部导航现在也按 B 站 App 的内容页风格处理：

- 左侧返回按钮保留页面回退
- 中间标题可点击，点击后打开原始 B 站网页
- 右侧保留 AI 图标按钮，直接进入 AI 笔记子路由
...
- 评论区域现在和其他模块一样，作为单列中的独立卡片渲染，不再出现在右侧信息栏
...
- 播放区和按钮区继续保留在详情页顶部流中，不会被拆成桌面右栏

### 7. 顶部导航与原网页跳转

- 视频详情页标题点击打开 `https://www.bilibili.com/video/{bvid}`
- 图文详情页标题点击打开对应的 `https://www.bilibili.com/read/cv{opusId}` 页面
- 跳转会新开标签页，不会离开 PiliNote
- 返回按钮在深色模式下使用更明显的按钮底和边框，避免和页面背景融在一起
...
- 图文正文和操作按钮也遵循同一套单列节奏，PC 与手机保持一致
...
### 2026-04-23 - 加载态与评论区收敛

#### 页面结构调整
- **作者与简介顺序调整**: 作者信息卡固定在封面下方，视频简介移动到作者面板下面
- **下载状态位置调整**: 下载状态组件从内容中部移到评论区之后，作为页面底部操作卡片
- **标题栏入口调整**: 右侧改为 AI 图标按钮，直接进入 AI 笔记子路由

#### 加载与错误态
- **骨架屏优先**: 刷新时先展示骨架屏，不再闪现纯文本“加载中...”
- **缓存兜底**: 详情接口短暂失败时，优先展示最近一次成功的缓存内容
- **错误态收敛**: 只有缓存也不可用且确实失败时，才显示错误页或“视频不存在”

#### 骨架动画
- **动画节奏优化**: 骨架 shimmer 更柔和，整体节奏更慢，并加入轻微错位，降低同步闪烁感

#### 评论区优化
- **数量移除**: 评论区标题右上角数量不再显示
- **视觉收敛**: 评论卡片改为更轻的背景与边框，去掉偏黑的面板感
- **颜色统一**: 评论标题、作者名与置顶徽标调整为更统一的中性色与强调色
