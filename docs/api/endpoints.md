...
本地联调时，认证接口和收藏夹 / 稍后再看 / 历史记录接口都可以复用 `apps/api/data/pilinote.db` 里的登录态。后端启动后会自动从数据库读取当前活跃账号的 cookies；如果用 `curl` 直测，则需要手动带上 `Cookie: SESSDATA=...; bili_jct=...`。
...
视频下载任务在完成媒体主体下载后，会自动尝试下载字幕文件。当前字幕链路遵循以下规则：

- 仅处理 B 站接口实际返回的 `subtitle_url`
- 官方 AI 字幕与普通字幕走同一条下载链路
- 当前下载逻辑只保留中英双语字幕：
  - `zh-CN`
  - `en-US`
- 同一视频可能返回多个字幕条目，系统会分别下载可用条目
- 如果条目存在但 `subtitle_url` 为空，会标记为“已检测到字幕，但不可下载”
