...
- `apps/web/src/pages/VideoDetailPage.css`

**更新内容**:
- 作者信息卡移动到封面下方
- 视频简介移动到作者信息卡下面
- 下载状态组件移动到评论区后面
- 标题栏右侧改为 AI 图标按钮，直接进入 AI 笔记子路由

##### 2. 加载态与缓存兜底
**更新内容**:
- 刷新时优先显示骨架屏，不再闪现纯文本“加载中...”
- 骨架屏使用更柔和、错位的 shimmer 动画
- 详情接口短暂失败时，优先复用最近一次成功的本地缓存
- 只有缓存也不可用且请求确实失败时，才显示错误页或“视频不存在”

##### 3. 评论区收敛
**更新内容**:
- 去掉评论区右上角数量
- 评论卡片背景与边框进一步收敛，去掉偏黑的面板感
- 评论区标题、作者名和置顶标签统一为更轻的中性色 / 强调色层级

#### 验证结果

- `apps/web` 的 `tsc --noEmit` 已通过

### 2026-04-23 - 视频详情页 B 站风格导航优化

本次更新继续收紧视频详情页的顶部导航体验，让它更像 B 站 App 的内容页。
...
**更新内容**:
- 顶栏改成更轻量的 App 风格 sticky 导航
- 返回按钮增加暗色模式下的可见度和边框层次
- 标题现在可点击，并会在新标签页打开原始 B 站网页
- 右侧补充一个显式的“原网页”入口，方便快速跳转

#### 验证结果

- 视频详情页和图文详情页已能在顶部显示标题外链与原网页入口
- 标题跳转不会离开 PiliNote 当前标签页

### 2026-04-23 - 视频详情页单列流式重做

本次更新把视频/图文详情页统一成 PC 与手机共用的单列流式布局，更接近 B 站 App 的详情页阅读节奏，同时保留所有原有功能入口。
...
**更新内容**:
- 去掉桌面双栏内容分支，PC 与手机共用同一套单列流式结构
- 顶部改为 sticky 返回栏，标题单行显示
- 封面 / 本地播放器、简介、评论、UP 主信息、统计、分 P 列表、下载按钮全部改为纵向卡片堆叠
- 图文（Opus）正文继续保持本地 Markdown / 远端内容兼容，但同样沿用单列节奏
- `<Outlet />` 继续保留在详情页底部，用于 AI 笔记子路由挂载

##### 2. 视觉收敛
**更新内容**:
- 顶部导航、卡片圆角、卡片间距、按钮高度做了统一收紧
- PC 端不再展示旧的右侧信息栏布局
- 保留滚动容器和自定义滚动条样式，页面仍然是独立全屏详情视图
